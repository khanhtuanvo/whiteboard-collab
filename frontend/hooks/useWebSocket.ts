...
export function useWebSocket({
  ...
  const emitBulkUpdateElements = useCallback((updates: BulkElementUpdate[]) => {
    return new Promise<void>((resolve, reject) => {
      getSocket().emit('elements:bulk_update', { boardId, updates }, (response: { ok: boolean; error?: string }) => {
        if (response?.ok) {
          resolve();
          return;
        }
        reject(new Error(response?.error ?? 'Failed to update elements'));
      });
    });
  }, [boardId]);
  ...
  return { emitCursorMove, emitCreateElement, emitUpdateElement, emitBulkUpdateElements, emitLiveUpdateElement, emitDeleteElement, emitUndo, emitRedo, emitClearBoard };
}
