...
interface BulkUpdateElementItem {
  elementId: string;
  properties: ElementProperties;
}

interface BulkUpdateElementData {
  boardId: string;
  userId: string;
  updates: BulkUpdateElementItem[];
}

interface BulkUpdateAck {
  ok: boolean;
  error?: string;
}
...
export class ElementEvents {
  ...
  async handleBulkUpdateElements(socket: Socket, io: Server, data: BulkUpdateElementData, ack?: (response: BulkUpdateAck) => void) {
    const { boardId, userId, updates } = data;
    ...
    try {
      ...
      if (!Array.isArray(updates) || updates.length === 0) {
        socket.emit('error', { message: 'No updates provided' });
        ack?.({ ok: false, error: 'No updates provided' });
        return;
      }
      ...
      const validatedUpdates = updates.map((update) => {
        if (!uuidSchema.safeParse(update.elementId).success) {
          throw new Error('Invalid element ID');
        }

        const rawPayload = update.properties as unknown as Record<string, unknown>;
        const incomingZIndex =
          typeof rawPayload.zIndex === 'number' && Number.isFinite(rawPayload.zIndex)
            ? rawPayload.zIndex
            : undefined;
        ...
        if (!propertiesResult.success) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (error as any).details = propertiesResult.error.issues;
          throw error;
        }
      ...
      const currentElements = await prisma.element.findMany({
        where: {
          boardId,
          id: { in: validatedUpdates.map((update) => update.elementId) },
        },
      });

      if (currentElements.length !== validatedUpdates.length) {
        socket.emit('error', { message: 'One or more elements not found' });
        ack?.({ ok: false, error: 'One or more elements not found' });
        return;
      }
      ...
      await prisma.$transaction(async (tx) => {
        for (const update of validatedUpdates) {
          const currentElement = elementMap.get(update.elementId);
          if (!currentElement) {
            throw new Error('Element not found');
          }

          const existing =
            typeof currentElement.properties === 'object' &&
            currentElement.properties !== null &&
            !Array.isArray(currentElement.properties)
          ...
          if (update.zIndex !== undefined) {
            updateData.zIndex = update.zIndex;
          }

          const updated = await tx.element.update({
            where: { id: update.elementId },
            data: updateData,
          });

          elementMap.set(update.elementId, updated);
        }
      });
      ...
      io.to(`board:${boardId}`).emit('element:snapshot', updatedElements);

      logger.info('Bulk element update applied', { boardId, userId, count: validatedUpdates.length });
      ack?.({ ok: true });
    } catch (error) {
      logger.error('Error applying bulk element updates', { boardId, userId, error });
      const message = error instanceof Error ? error.message : 'Failed to update elements';
