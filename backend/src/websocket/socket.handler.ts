...
export function setupSocketHandlers(io: Server) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('elements:bulk_update', async (data, ack) => {
      if (!await rateGuard('elements:bulk_update')) {
        ack?.({ ok: false, error: 'Rate limit exceeded' });
        return;
      }
      elementEvents.handleBulkUpdateElements(socket, io, {
        ...data,
        userId,
      }, ack);
    });
