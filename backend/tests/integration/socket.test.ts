...
const SECOND_ELEMENT_ID = 'd4c3b2a1-f6e5-4987-abcd-654321fedcba';
...
describe('Socket.io event pipeline', () => {
  ...
  it('acknowledges success after bulk update and persists updated board state', async () => {
    (prisma.board.findFirst as jest.Mock).mockResolvedValue(mockBoard);

    const existingElements = [
      { ...mockElement, id: ELEMENT_ID, properties: { x: 10, y: 20, width: 100, height: 50, fill: '#3b82f6' }, zIndex: 1 },
      { ...mockElement, id: SECOND_ELEMENT_ID, properties: { x: 40, y: 50, width: 120, height: 60, fill: '#22c55e' }, zIndex: 2 },
    ];

    const updatedElements = [
      { ...existingElements[0], properties: { x: 100, y: 120, width: 100, height: 50, fill: '#3b82f6' } },
      { ...existingElements[1], properties: { x: 140, y: 160, width: 120, height: 60, fill: '#22c55e' } },
    ];

    (prisma.element.findMany as jest.Mock)
      .mockResolvedValueOnce(existingElements)
      .mockResolvedValueOnce(existingElements)
      .mockResolvedValueOnce(updatedElements);

    (prisma.$transaction as jest.Mock).mockImplementation(async (fn: any) => {
      return fn({
        element: {
          update: jest.fn()
            .mockResolvedValueOnce(updatedElements[0])
            .mockResolvedValueOnce(updatedElements[1]),
        },
      });
    });

    const redisMock = jest.requireMock('../../src/config/redis').default as { eval: jest.Mock; llen: jest.Mock };
    redisMock.eval.mockReset();
    // First eval call is the rate-limit check for elements:bulk_update.
    // Second eval call is saveSnapshot Lua.
    redisMock.eval
    ...
    const ackPromise = new Promise<{ ok: boolean; error?: string }>((resolve) => {
      ownerSocket.emit(
        'elements:bulk_update',
        {
          boardId: BOARD_ID,
          updates: [
            { elementId: ELEMENT_ID, properties: { x: 100, y: 120 } },
            { elementId: SECOND_ELEMENT_ID, properties: { x: 140, y: 160 } },
          ],
        },
  ...
  it('returns ack error when bulk update is rate-limited', async () => {
    (prisma.board.findFirst as jest.Mock).mockResolvedValue(mockBoard);

    const redisMock = jest.requireMock('../../src/config/redis').default as { eval: jest.Mock };
    redisMock.eval.mockReset();
    // First call is the rate-limit check for elements:bulk_update.
    redisMock.eval.mockResolvedValueOnce(999);

    ownerSocket.connect();
    await waitForConnect(ownerSocket);

    const socketErrorPromise = waitForEvent<{ message: string; event: string }>(ownerSocket, 'error');
    const ack = await new Promise<{ ok: boolean; error?: string }>((resolve) => {
      ownerSocket.emit(
        'elements:bulk_update',
        {
          boardId: BOARD_ID,
          updates: [
            { elementId: ELEMENT_ID, properties: { x: 50, y: 60 } },
          ],
        },
    ...
    const socketError = await socketErrorPromise;

    expect(ack).toEqual({ ok: false, error: 'Rate limit exceeded' });
    expect(socketError).toEqual({ message: 'Rate limit exceeded', event: 'elements:bulk_update' });
