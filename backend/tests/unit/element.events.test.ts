...
// =============================================================================
// Bulk update — AI apply path
// =============================================================================
describe('handleBulkUpdateElements', () => {
  it('applies updates in a transaction, emits snapshot, and acknowledges success', async () => {
    grantAccess();

    const secondElementId = 'dddddddd-dddd-4ddd-8ddd-dddddddddddd';
    const existingElements = [
      { ...MOCK_ELEMENT, id: ELEM_ID, properties: { x: 10, y: 20, fill: 'red' }, zIndex: 1 },
      { ...MOCK_ELEMENT, id: secondElementId, properties: { x: 30, y: 40, fill: 'blue' }, zIndex: 2 },
    ];
    const preSnapshot = [...existingElements];
    const finalSnapshot = [
      { ...existingElements[0], properties: { x: 110, y: 120, fill: 'red' } },
      { ...existingElements[1], properties: { x: 130, y: 140, fill: 'blue' } },
    ];
    ...
    const updateMock = jest.fn()
      .mockResolvedValueOnce(finalSnapshot[0])
      .mockResolvedValueOnce(finalSnapshot[1]);
    ...
    await events.handleBulkUpdateElements(
      socket as unknown as Socket,
      { to } as unknown as Server,
      {
        boardId: BOARD_ID,
        userId: USER_ID,
        updates: [
          { elementId: ELEM_ID, properties: { x: 110, y: 120 } },
          { elementId: secondElementId, properties: { x: 130, y: 140 } },
        ],
      },
      ack
    );
    ...
    expect(roomEmit).toHaveBeenCalledWith('element:snapshot', finalSnapshot);
    expect(ack).toHaveBeenCalledWith({ ok: true });
  });

  it('rejects empty bulk payloads and acknowledges failure', async () => {
    grantAccess();
    ...
    await events.handleBulkUpdateElements(
      socket as unknown as Socket,
      { to } as unknown as Server,
      { boardId: BOARD_ID, userId: USER_ID, updates: [] },
      ack
    );

    expect(socket.emit).toHaveBeenCalledWith('error', { message: 'No updates provided' });
    expect(ack).toHaveBeenCalledWith({ ok: false, error: 'No updates provided' });
    expect(prisma.$transaction).not.toHaveBeenCalled();
  });

  it('rejects invalid properties in bulk updates and acknowledges failure', async () => {
    grantAccess();
    ...
    await events.handleBulkUpdateElements(
      ...
      { to } as unknown as Server,
      {
        ...
        userId: USER_ID,
        updates: [
          { elementId: ELEM_ID, properties: { x: Number.NaN, y: 10 } },
        ],
      },
      ack
    );

    expect(socket.emit).toHaveBeenCalledWith('error', { message: 'Invalid properties' });
    expect(ack).toHaveBeenCalledWith({ ok: false, error: 'Invalid properties' });
    expect(prisma.$transaction).not.toHaveBeenCalled();
  });
});
